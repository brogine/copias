...
class ESaveRelatedBehavior extends CActiveRecordBehavior
{
    ...
    public function deleteRelated($relations)
    {
        if ($this->owner->isNewRecord) {
            throw new CException("Function deleteRelated() cannot be used for new objects.");
        }
        return $this->deleteR($relations);
    }

    protected function deleteR($relations)
    {
        $result = true; $t = false;
        ...
        // loop through all relations that should be saved
        foreach ((array)$relations as $key => $relationName)
        {
            // get relation information
            $relation = $this->owner->getActiveRelation($relationName);
            // get the data that was set for this relation, if no data was set, $data will contain the current related records
            $data = $this->owner->$relationName;
            ...
            if ($relation instanceof CManyManyRelation)
            {
                // extract infos about mn linking table
                ...
                foreach($data as $id)
                {
                    // get id if object was given
                    if (is_object($id))
                        $id = $id->primaryKey;

                    $criteria = new CDbCriteria();
                    $criteria->compare($info['mnFk1'], $this->owner->primaryKey);
                    $criteria->compare($info['mnFk2'], $id);

                    $affected = $commandBuilder->createDeleteCommand($info['mnTable'],$criteria)->execute();

                    if ($affected <= 0)
                        $result = false;
                }

                // saving was successful, clear the relation, so accessing it will return the related records
                if ($result)
                    unset($this->owner->$relationName);
            }
        }

        // commit on success if transaction was started in this behavior
        if ($t && $result)
            $t->commit();

        // rollback on errors if transaction was started in this behavior
        if ($t && !$result)
            $t->rollback();
